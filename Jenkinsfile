...
pipeline {
  ...
  stages {
        stage ('git checkout'){
            steps{
                git branch: "${github_branch}", changelog: false, credentialsId: 'github', poll: false, url: "https://${github_repo}"
            }
        }
        ...
        stage ('upload_to_nexus'){
     		steps{
				nexusArtifactUploader artifacts: [[artifactId: "${project_name}", classifier: '', file: "${file_path}", type: 'war']], credentialsId: 'Nexus', groupId: "${project_name}", nexusUrl: "${nexus_path}", nexusVersion: 'nexus3', protocol: 'http', repository: "${nexus_repo_name}", version: "${version}" 
            }
          }
